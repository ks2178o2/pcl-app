...
class TestValidationMiddleware:
    ...
    async def test_validate_rag_feature_exists_success(self, mock_supabase_client):
        ...
        # Mock feature exists
        mock_supabase_client.from_.return_value.select.return_value.eq.return_value.single.return_value.execute.return_value.data = [SAMPLE_RAG_FEATURES[0]]
        
        with patch('middleware.validation.get_supabase_client', return_value=mock_supabase_client):
            result = await validate_rag_feature_exists('best_practice_kb')
            assert result is True
    ...
    async def test_validate_rag_feature_exists_not_found(self, mock_supabase_client):
        ...
        mock_supabase_client.from_.return_value.select.return_value.eq.return_value.single.return_value.execute.side_effect = Exception("0 rows")
        
        with patch('middleware.validation.get_supabase_client', return_value=mock_supabase_client):
            with pytest.raises(Exception):  # Should raise HTTPException
                await validate_rag_feature_exists('nonexistent_feature')
    ...
    async def test_validate_organization_hierarchy_success(self, mock_supabase_client):
        ...
        with patch('middleware.validation.get_supabase_client', return_value=mock_supabase_client):
            result = await validate_organization_hierarchy('org-child', 'org-parent')
            assert result is True
    ...
    async def test_validate_organization_hierarchy_circular_dependency(self, mock_supabase_client):
        ...
        mock_supabase_client.from_.return_value.select.return_value.eq.return_value.execute.return_value.data = [
            {'ancestor_id': 'org-child'}
        ]
        
        with patch('middleware.validation.get_supabase_client', return_value=mock_supabase_client):
            with pytest.raises(Exception):  # Should raise HTTPException
                await validate_organization_hierarchy('org-child', 'org-parent')
