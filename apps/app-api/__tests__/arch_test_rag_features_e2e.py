...
class TestRAGFeatureE2EWorkflows:
    ...
    async def test_feature_validation_workflow(self, setup_e2e_environment):
        ...
        rag_validator.supabase = env['mock_client']

        result = await rag_validator.validate_rag_feature_exists('invalid_feature_name')
        assert result is False

        env['mock_client'].from_.return_value.select.return_value.eq.return_value.execute.return_value.data = [
            {'parent_organization_id': 'org-child-1'}
        ]

        is_valid, message = await rag_validator.validate_org_hierarchy('org-child-1', 'org-parent')
