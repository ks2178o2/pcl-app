...
export class ChunkedRecordingManager {
  ...
  private recorderMime: string = "audio/ogg;codecs=opus";
  private fileExt: "ogg" | "webm" = "ogg";

  // 5s resilience slices for crash recovery
  ...
  // in-flight sequence for IndexedDB slices
  private currentSliceSeq = 0;
  
  // Legacy chunk tracking (for backward compatibility)
  private currentChunkNumber: number = 0;
  private chunks: ChunkInfo[] = [];
  private chunkStartTime?: number;
  private _chunkHardStopTimer: number | null = null;
  private chunkDuration: number = 5 * 60 * 1000; // 5 minutes
  private pendingUploads: Map<number, Promise<void>> = new Map();
  ...
  private bindLifecycleGuards() {
    ...
    const vis = () => {
      if (document.visibilityState === "hidden") {
        // Flush current data and save state
        try {
          this.mediaRecorder?.requestData();
        } catch {}
        this.saveState();
        ...
        console.log("👁️ Tab visible - recording continues");
        ...
        if (this.isRecording && this.mediaRecorder?.state === "recording") {
          try {
            ...
            console.error("Failed to flush audio data:", e);
          }
        }
  ...
  async stopRecording(): Promise<void> {
    ...
    if (this.mediaRecorder?.state === "recording") {
      ...
      this.mediaRecorder.stop(); // finalize - triggers onstop → schedules final chunk upload
    }
    if (this.stream) {
    ...
    // CRITICAL: Wait for all uploads including the final chunk scheduled by onstop
    console.log("⏳ Waiting for all pending uploads before marking complete...");
    await this.waitForAllUploads();
    console.log("✅ All uploads confirmed, marking recording as complete");

    // ONLY NOW mark as complete after all chunks are uploaded
    if (this.callRecordId) await this.updateCallRecordCompletion();
  ...
  private async startNextChunk(): Promise<void> {
    ...
    // Prefer Ogg so concatenation is container-safe
    let mimeType = ogg;
    if (!(window as any).MediaRecorder?.isTypeSupported?.(ogg)) {
      console.warn("Ogg/Opus not supported here; falling back to WebM/Opus");
      mimeType = webm;
      this.fileExt = "webm";
      this.recorderMime = webm;
    } else {
      this.fileExt = "ogg";
      this.recorderMime = ogg;
    }
  ...
  private async uploadChunk(chunkInfo: ChunkInfo): Promise<boolean> {
    ...
    // Create upload promise and register it
    const uploadPromise = (async () => {
      ...
      try {
        ...
        const fileName = `${user.id}/${chunkInfo.callRecordId}/chunk-${chunkInfo.chunkNumber}.${this.fileExt}`;

        const contentType = this.fileExt === "ogg" ? "audio/ogg" : "audio/webm";

        const { data: uploadData, error: uploadError } = await supabase.storage
          .from("call-recordings")
          .upload(fileName, chunkInfo.audioBlob, { contentType, upsert: true });
        ...
        console.log(
          `✅ Chunk ${chunkInfo.chunkNumber} uploaded to storage and recorded in DB at ${new Date().toISOString()}`,
        );
        ...
        const isOffline =
          typeof navigator !== "undefined" && "onLine" in navigator && (navigator as any).onLine === false;
    ...
    // Track this upload in the pending map
    this.pendingUploads.set(chunkInfo.chunkNumber, uploadPromise.then(() => {}));

    // Clean up from map when done
    uploadPromise.finally(() => {
      this.pendingUploads.delete(chunkInfo.chunkNumber);
    });

    return uploadPromise;
  }

  private async waitForAllUploads(timeoutMs = 60000): Promise<void> {
    if (this.pendingUploads.size === 0) {
      console.log("✅ No pending uploads to wait for");
      return;
    }

    console.log(`⏳ Waiting for ${this.pendingUploads.size} pending uploads...`);
    const start = Date.now();

    try {
      // Race between all uploads completing and timeout
      await Promise.race([
        Promise.all([...this.pendingUploads.values()]),
        new Promise((_, reject) => setTimeout(() => reject(new Error("Upload timeout")), timeoutMs)),
      ]);

      const elapsed = Date.now() - start;
      console.log(`✅ All uploads completed in ${elapsed}ms`);
    } catch (error) {
      console.error("⚠️ Some uploads may have failed or timed out:", error);
